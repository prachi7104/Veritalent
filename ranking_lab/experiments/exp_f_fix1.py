# ranking_lab/experiments/exp_f_fix1.py
"""
Experiment F Fix-1: 10-feature set, activity constrained.
Gate: NDCG@10 >= 0.7473
Success means: stop here, ship this model.
Failure means: run Fix-2.
"""
...
from ranking_lab.models.feature_config_v2_fix1 import FEATURE_NAMES_FIX1, MONOTONIC_CONSTRAINTS_FIX1
...
def train_eval(store, labels, gold, feats, constrs, seed):
    ...
    # Construct model and inject custom features/constraints
...
def run():
    print("\n=== Experiment F Fix-1: 10-feature, activity constrained ===")
    print(f"Removed: skill_recency, implied_skill_score")
    print(f"Changed: activity_quality_composite constraint 0 -> +1")
    ...
    for seed in [42, 7, 123]:
        ndcg, m, ranked = train_eval(
            store, labels, gold, FEATURE_NAMES_FIX1, MONOTONIC_CONSTRAINTS_FIX1, seed
        )
    ...
    if sp_42_7 == 1.0 and sp_42_123 == 1.0:
        print("  [Warning] Spearman still 1.0 - model still collapsed. Proceed to Fix-2.")

    # Primary model
    primary_ndcg, primary_m, primary_ranked = train_eval(
        store, labels, gold, FEATURE_NAMES_FIX1, MONOTONIC_CONSTRAINTS_FIX1, 42
    )
    ...
    print("\n  Feature Importances:")
    for feat, imp in sorted(zip(FEATURE_NAMES_FIX1, normalized_imps), key=lambda x: -x[1]):
        bar = "#" * int(imp * 50)
    ...
    # First try loading from candidates.jsonl since it is complete
    ...
    if comp["verdict"] != "REGRESSION":
        ...
        print("\n  [FAIL] Fix-1 failed. Proceed to Fix-2.")

    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    report = {
        "experiment": "exp_f_fix1",
        "features": FEATURE_NAMES_FIX1,
        "ndcg@10": primary_ndcg,
        "comparison": comp,
        "spearman_42_7": sp_42_7,
        "spearman_42_123": sp_42_123,
        "feature_importances": dict(zip(FEATURE_NAMES_FIX1, imps.tolist())),
    }
    out = Path(f"reports_archive/exp_f_fix1_{ts}.json")
